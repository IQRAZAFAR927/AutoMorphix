...
function Accept_CarModels() {
  const [models, setModels] = useState([]);

  useEffect(() => {
    axios.get('http://localhost:3000/model/getacceptmodels')
      .then(response => {
  ...
  return (
    <div className="d-flex flex-column" style={{ minHeight: '100vh' }}>
      ...
      <div className="container mt-5 pt-5">
        <h3 className="mb-4 text-center" style={{paddingTop:'30px', paddingBottom:'20px'}}>Accept Car Models</h3>
        <div className="row">
          {models.map(model => (
            <div key={model._id} className="col-md-4 mb-4">
              <div className={`card ${model.status === 'accepted' ? 'border-success' : model.status === 'rejected' ? 'border-danger' : ''}`} style={{ transition: 'border 0.3s' }}>
                <div className="card-body" 
                  ...
                  <p className="card-text"><strong>Comment:</strong>  <span style={{ color: 'green' }}></span>{model.Comments}</p>
...
export default Accept_CarModels;
